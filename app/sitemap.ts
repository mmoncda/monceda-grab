...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return paths.map((path, index) => ({ url: `${base}${path}`, lastModified: new Date("2026-08-11"), changeFrequency: index === 0 ? "weekly" : "monthly", priority: index === 0 ? 1 : path === "/guides" ? 0.8 : 0.7 }));
}
